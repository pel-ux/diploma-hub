...
function HOCDashboard() {
  ...
  const firstName = userData?.name?.split(" ")[0] || user?.email?.split("@")[0] || "HOC"

  const roleBadge = (role) => {
    const map = {
      student: "bg-blue-50 text-blue-700",
      lecturer: "bg-amber-50 text-amber-700",
      hoc: "bg-green-50 text-green-700"
    }
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      `}>
        <div>
          ...
          <div className="bg-white/10 rounded-lg px-3 py-2 mb-6">
            <p className="text-amber-400 text-xs font-medium">HOC Portal</p>
          </div>
        ...
        <div className="border-t border-white/10 pt-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-amber-400 text-xs">Head of Course</p>
            </div>
          </div>
          <button
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Head of Course — Sciences Diploma Programme</p>
            </div>
          </div>
          <div className="w-8 h-8 rounded-full bg-amber-400 flex items-center justify-center text-blue-900 text-sm font-bold">
            {firstName[0]?.toUpperCase()}
        ...
        <main className="flex-1 p-6">
          ...
          {activePage === "dashboard" && (
            <div>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                {[
                  { label: "Total Users", value: users.length || "—", icon: "👥", color: "bg-blue-50 text-blue-900" },
                  { label: "Notifications", value: notifications.length, icon: "🔔", color: "bg-amber-50 text-amber-700" },
                  { label: "Results Uploaded", value: "—", icon: "📊", color: "bg-green-50 text-green-700" },
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white rounded-2xl border border-gray-100 p-6">
                  ...
                  <div className="space-y-3">
                    {[
                      { label: "Post Notification", sub: "Announce to all students", icon: "🔔", action: () => setActivePage("notifications") },
                      { label: "Upload Results", sub: "Upload CSV result file", icon: "📊", action: () => setActivePage("results") },
                      { label: "Manage Users", sub: "View and promote users", icon: "👥", action: () => setActivePage("users") },
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
                <div>
                  ...
                  <input
                    type="file"
                    accept=".csv"
                    className="w-full px-4 py-2.5 rounded-lg border border-gray-200 bg-gray-50 text-sm outline-none transition-colors"
          ...
          {activePage === "users" && (
            <div>
              <h2 className="text-xl font-semibold text-blue-900 mb-6">Manage Users</h2>
                    ...
                    <tbody>
                      {users.map(u => (
                        <tr key={u.uid} className="border-b border-gray-50 hover:bg-gray-50">
                          ...
                          <td className="px-5 py-3 text-gray-600">{u.studentId || "—"}</td>
                          ...
                          <td className="px-5 py-3">
                            <select
                              value={u.role}
                              onChange={(e) => promoteUser(u.uid, e.target.value)}
                              className="text-xs border border-gray-200 rounded-lg px-2 py-1.5 outline-none focus:border-blue-900 bg-white"
                            >
